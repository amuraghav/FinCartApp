...
class GetOTPVC: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var nextButton: RoundRectUIButton!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        setUpTextFields(emailIdTextField, "ic_email_profile")
        setUpTextFields(otpTextField, "ic_key")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let height = nextButton.frame.origin.y + nextButton.frame.size.height
    ...
    @IBAction func nextButtonAction(_ sender: UIButton) {
        if !Validation.validateEmail(emailIdTextField.text){
        ...
        }else if !Validation.validateOTP(otpTextField.text){
            let alert = UIAlertController(title: "Invalid OTP", message: "Please enter OTP.", preferredStyle: UIAlertControllerStyle.alert)
            self.present(alert, animated: true)
            // duration in seconds
            let duration: Double = 0.5
            ...
            appDelegate.showResetPasswordScreen()
        }
    }
