...
class FinCartQuestionnaireVC: FinCartViewController, UITextViewDelegate{
    
    private var optionHeight: CGFloat?
    var jsonFileName : String?
    var jsonObject : JsonBase?
    var indexCount: Int = 0
    var indexSubQuestionArray: Int = 0
    var quesAnsArray = [Int:String]()
    var subQuesAnsArray = [Int:[Int:String]]()
    var ansText: String?
    var isSubQuestion: Bool = false;
    var questionFinished = false;
    var isPreviousFlow = false;
    var isNextFlow = true
    var latestQuesIndex = [Int]()
    var latestSubQuestionIndex = Dictionary<Int, [Int]>()
    var parentAnswerArray = [String]()
    var yesCount: Int?
    ...
    override func viewDidLoad() {
        ...
        getDefaultOptions()
        getOptionHeight()
        displayAnsOptionType(_ansType: (jsonObject?.baseObject?[indexCount].anstype)!, _quesString: (jsonObject?.baseObject?[0].ques)!, _optionArray: (jsonObject?.baseObject?[indexCount].options)!, _isSubQuestion: false, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
        latestQuesIndex.append(0)
        latestSubQuestionIndex.updateValue([], forKey: 0)
    ...
    {
        resultString = sender.titleLabel?.text
        performNextAction()
    }
    ...
    private func getDefaultOptions(){
        ...
        {
            let defaultAnsOption = (jsonObject?.baseObject?[index].defaultOption)!
            if defaultAnsOption.isEmpty{
                quesAnsArray.updateValue("", forKey: index)
                parentAnswerArray.insert("", at: index)
            }
            else{
                for quesIndex in 0 ..< (jsonObject?.baseObject?[index].options?.count)!{
                    if jsonObject?.baseObject?[index].options?[quesIndex].key?.caseInsensitiveCompare(defaultAnsOption) == ComparisonResult.orderedSame{
                        quesAnsArray.updateValue((jsonObject?.baseObject?[index].options?[quesIndex].value)!, forKey: index)
                        parentAnswerArray.insert((jsonObject?.baseObject?[index].options?[quesIndex].value)!, at: index)
                    }
                }
            }
        }
        for index in 0 ..< ((jsonObject?.baseObject?.count)!){
            ...
            for subIndex in 0 ..< ((jsonObject?.baseObject?[index].quesArray?.count)!)
            {
                let defaultAnsOption = (jsonObject?.baseObject?[index].quesArray?[subIndex].defaultOption)!
                if defaultAnsOption.isEmpty{
                    subDictionary.updateValue("", forKey: subIndex)
                }
                else{
                    for quesIndex in 0 ..< (jsonObject?.baseObject?[index].options?.count)!{
                        if jsonObject?.baseObject?[index].quesArray?[subIndex].options?[quesIndex].key?.caseInsensitiveCompare(defaultAnsOption) == ComparisonResult.orderedSame{
                            subDictionary.updateValue((jsonObject?.baseObject?[index].quesArray?[subIndex].options?[quesIndex].value)!, forKey: subIndex)
                        }
                    }
                }
            }
            subQuesAnsArray.updateValue(subDictionary, forKey: index)
        }
    }
    
    private func displayAnsOptionType(_ansType: String, _quesString: String, _optionArray: [OptionsArray], _isSubQuestion: Bool, _indexCount: Int, _subIndexCount: Int)
    {
        ...
        if _ansType.caseInsensitiveCompare("checkbox") == ComparisonResult.orderedSame {
            if _optionArray.count > 2{
                setUpCheckBoxes(_optionArray, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
            }
            else{
                setUpCheckBoxesForYesNoOption(_optionArray, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
            }
        }
        else if _ansType.caseInsensitiveCompare("incredecre") == ComparisonResult.orderedSame {
            setUpIncrementButtons(_optionArray, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
        }
        else if _ansType.caseInsensitiveCompare("scrollincredecre") == ComparisonResult.orderedSame{
            setUpScrollIncrement(_optionArray, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
        }
        else if _ansType.caseInsensitiveCompare("textfield") == ComparisonResult.orderedSame {
            setUpTextField(_optionArray, _isSubQuestion: _isSubQuestion)
        }
    }
    
    private func updatingPreviousQues(){
        isPreviousFlow = true
        if latestQuesIndex.count > 0 {
            if isNextFlow{
                indexCount = latestQuesIndex.last!
                indexSubQuestionArray = -1
                if (latestSubQuestionIndex[indexCount]?.count)! > 0 {
                    indexSubQuestionArray = (latestSubQuestionIndex[indexCount]?.last)!
                }else{
                    latestQuesIndex.removeLast()
                }
                if indexSubQuestionArray == 0{
                    latestQuesIndex.removeLast()
                }
            }
            indexSubQuestionArray = 0
            if latestQuesIndex.count > 0{
                indexCount = latestQuesIndex.last!
            }else{
                indexCount = 0
            }
            let answerText = quesAnsArray[indexCount]
            if answerText?.caseInsensitiveCompare("YES") != ComparisonResult.orderedSame{
                displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].options)!, _isSubQuestion: false, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
                latestQuesIndex.removeLast()
                parentAnswerArray.removeLast()
                isSubQuestion = false;
            }else{
                if (latestSubQuestionIndex[indexCount]?.count)! > 0 {
                    indexSubQuestionArray = (latestSubQuestionIndex[indexCount]?.last)!
                    displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].quesArray?[indexSubQuestionArray].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].quesArray?[indexSubQuestionArray].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].quesArray![indexSubQuestionArray].options)!, _isSubQuestion: true, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
                    latestSubQuestionIndex[indexCount]?.removeLast()
                    isSubQuestion = true
                }else{
                    displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].options)!, _isSubQuestion: false, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
                    latestQuesIndex.removeLast()
    ...
    private func updatingNextQues(){
        ...
        if isPreviousFlow && isSubQuestion{
            if latestSubQuestionIndex[indexCount] != nil
            {
                var set = latestSubQuestionIndex[indexCount]
                set?.append(indexSubQuestionArray)
                latestSubQuestionIndex.updateValue(set!, forKey: indexCount)
            }
            indexSubQuestionArray = indexSubQuestionArray + 1
        }else if isPreviousFlow{
            latestQuesIndex.append(indexCount)
        }
    
        isPreviousFlow = false
        nextButton.isEnabled = false
        //isSubQuestionFinished = false
        ansText = quesAnsArray[indexCount]!
        if (jsonObject?.baseObject![indexCount].anstype?.caseInsensitiveCompare("checkbox") == ComparisonResult.orderedSame) && (ansText?.caseInsensitiveCompare("NO") == ComparisonResult.orderedSame){
            indexSubQuestionArray = (jsonObject?.baseObject![indexCount].quesArray?.count)!
        }
        if indexSubQuestionArray < (jsonObject?.baseObject![indexCount].quesArray?.count)! && !isPreviousFlow{
            displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].quesArray?[indexSubQuestionArray].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].quesArray?[indexSubQuestionArray].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].quesArray![indexSubQuestionArray].options)!, _isSubQuestion: true, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
            if latestSubQuestionIndex[indexCount] != nil
            {
                var set = latestSubQuestionIndex[indexCount]
                ...
                latestSubQuestionIndex.updateValue([indexSubQuestionArray], forKey: indexCount)
            }
            indexSubQuestionArray = indexSubQuestionArray + 1;
            if jsonObject?.baseObject?[indexCount].goalcode?.compare("FG14") == ComparisonResult.orderedSame{
                if indexSubQuestionArray == (jsonObject?.baseObject![indexCount].quesArray?.count)! - 1{
        ...
        else if (indexCount < (jsonObject?.baseObject?.count)! - 1){
            indexSubQuestionArray = 0;
            isSubQuestion = false
            let countedSet = NSCountedSet(array: parentAnswerArray)
            let Keys = countedSet.count(for: "YES")
            if ((yesCount! + Keys) == 0 && indexCount >= (jsonObject?.baseObject?.count)! - 2){
                indexCount = (jsonObject?.baseObject?.count)! - 1
                displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].options)!, _isSubQuestion: false, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
                
                latestQuesIndex.append(indexCount)
                latestSubQuestionIndex.updateValue([], forKey: indexCount)
            }
            else if ((yesCount! + Keys) >= 3 && (!(jsonObject?.baseObject?[indexCount].questioncode?.compare("FQ36") == ComparisonResult.orderedSame)))
            {
                indexCount = (jsonObject?.baseObject?.count)! - 2
                displayAnsOptionType(_ansType: (jsonObject?.baseObject![indexCount].anstype)!, _quesString: (jsonObject?.baseObject![indexCount].ques)!, _optionArray: (jsonObject?.baseObject![indexCount].options)!, _isSubQuestion: false, _indexCount: indexCount, _subIndexCount: indexSubQuestionArray)
                latestQuesIndex.append(indexCount)
                latestSubQuestionIndex.updateValue([], forKey: indexCount)
                questionFinished = true
            }
            else if indexCount < (jsonObject?.baseObject?.count)! - 2
            {
    ...
    private func setUpCheckBoxesForYesNoOption(_ optionArray: [OptionsArray], _isSubQuestion: Bool, _indexCount: Int, _subIndexCount: Int)
    {
        optionsViewWidthConstraint.constant = 208
        optionsViewHeightConstraint.constant = optionHeight! + 10
        let width = (188)/2
        for view in self.optionsView.subviews{
            optionsView.willRemoveSubview(view)
            view.removeFromSuperview()
        ...
        if jsonObject?.baseObject?[indexCount].goalcode?.caseInsensitiveCompare(FincartCommon.GOAL_BANK_BALANCE) == ComparisonResult.orderedSame{
            textValue = self.getSavingFromIncome(income!, rate: 0.1)
        }
        let firstRadioButton: DLRadioButton = createRadioButton(CGRect(x: 0, y: 0, width: width, height: Int(optionHeight!)), _text: textValue, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
        ...
        var vertialSpacing = NSLayoutConstraint(item: firstRadioButton, attribute: .centerY, relatedBy: .equal, toItem: optionsView, attribute: .centerY, multiplier: 1.0, constant: 0)
        ...
        let widthConstraint = NSLayoutConstraint(item: firstRadioButton, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: CGFloat(width))
        firstRadioButton.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([vertialSpacing, leadingConstraint, widthConstraint, heightConstraint])
        buttonArray.add(firstRadioButton)
        
        for index in 1 ..< (optionArray.count){
            textValue = optionArray[index].value!
            if jsonObject?.baseObject?[indexCount].goalcode?.caseInsensitiveCompare(FincartCommon.GOAL_BANK_BALANCE) == ComparisonResult.orderedSame{
                textValue = self.getSavingFromIncome(income!, rate: 0.1 * Double(index + 1))
            }
            let secondRadioButton: DLRadioButton = createRadioButton(CGRect(x: 0, y: 0, width: width, height: Int(optionHeight!)), _text: textValue, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
            optionsView.addSubview(secondRadioButton)
            leadingConstraint = NSLayoutConstraint(item: secondRadioButton, attribute:.leading, relatedBy: .equal, toItem: buttonArray[index - 1], attribute: .trailing, multiplier: 1.0, constant: 20)
            vertialSpacing = NSLayoutConstraint(item: secondRadioButton, attribute: .centerY, relatedBy: .equal, toItem: optionsView, attribute: .centerY, multiplier: 1.0, constant: 0)
            ...
            let trailingConstraint = NSLayoutConstraint(item: secondRadioButton, attribute: .trailing, relatedBy: .equal, toItem: optionsView, attribute: .trailing, multiplier: 1.0, constant: 0)
            secondRadioButton.translatesAutoresizingMaskIntoConstraints = false
            NSLayoutConstraint.activate([leadingConstraint, vertialSpacing, heightConstraint, trailingConstraint])
    ...
    private func setUpCheckBoxes(_ optionArray: [OptionsArray], _isSubQuestion: Bool, _indexCount: Int, _subIndexCount: Int){
        ...
        let firstRadioButton: DLRadioButton = createRadioButton(CGRect(x: 0, y: 0, width: width, height: optionHeight!), _text: textValue, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
        optionsView.addSubview(firstRadioButton)
        var leadingConstraint = NSLayoutConstraint(item: firstRadioButton, attribute:.leading, relatedBy: .equal, toItem: optionsView, attribute: .leading, multiplier: 1.0, constant: 0)
        ...
        for index in 1 ..< (optionArray.count){
            ...
            let secondRadioButton: DLRadioButton = createRadioButton(CGRect(x: 0, y: 0, width: width, height: optionHeight!), _text: textValue, _isSubQuestion: _isSubQuestion, _indexCount: _indexCount, _subIndexCount: _subIndexCount)
            optionsView.addSubview(secondRadioButton)
    ...
    private func setUpScrollIncrement(_ optionArray: [OptionsArray], _isSubQuestion: Bool, _indexCount: Int, _subIndexCount: Int){
        optionsViewWidthConstraint.constant = self.view.frame.width - 32
        ...
        var defaultValue = ""
        if _isSubQuestion {
            defaultValue = subQuesAnsArray[_indexCount]![_subIndexCount]!
        }
        else{
    ...
    private func setUpIncrementButtons(_ optionArray: [OptionsArray], _isSubQuestion: Bool, _indexCount: Int, _subIndexCount: Int){
        optionsViewWidthConstraint.constant = self.view.frame.width - 32
        let width = self.view.frame.width - 32
        optionsViewHeightConstraint.constant = optionHeight! + 5
        for view in self.optionsView.subviews{
        ...
        if _isSubQuestion {
            resultString = subQuesAnsArray[_indexCount]![_subIndexCount]
            incrementLabel?.text = resultString
        }
        else{
            resultString = quesAnsArray[_indexCount]
            incrementLabel?.text = resultString
        }
    ...
    private func setUpTextField(_ optionArray: [OptionsArray], _isSubQuestion: Bool){
        optionsViewWidthConstraint.constant = self.view.frame.width - 32
        let width = self.view.frame.width - 32
        for view in self.optionsView.subviews{
            optionsView.willRemoveSubview(view)
        ...
        optionsViewHeightConstraint.constant = height + 10
    ...
    private func createRadioButton(_ frame: CGRect, _text: String, _isSubQuestion: Bool, _indexCount: Int, _subIndexCount:Int) -> DLRadioButton{
        ...
        if _isSubQuestion {
            if subQuesAnsArray[_indexCount]![_subIndexCount]?.caseInsensitiveCompare(radioButton.accessibilityIdentifier!) == ComparisonResult.orderedSame{
                resultString = subQuesAnsArray[_indexCount]![_subIndexCount]
            ...
            if quesAnsArray[_indexCount]?.caseInsensitiveCompare(radioButton.accessibilityIdentifier!) == ComparisonResult.orderedSame{
                resultString = quesAnsArray[_indexCount]
                radioButton.isSelected = true
                nextButton.isEnabled = true
            }
    ...
    private func tagQuestions(_ index: Int, subIndex: Int, child: Int)
    {
        if isSubQuestion{
            var subDictionary = (subQuesAnsArray[index])! as [Int:String]
            subDictionary.updateValue(resultString!, forKey: subIndex)
            subQuesAnsArray.updateValue(subDictionary, forKey: index)
        }
        else{
            quesAnsArray.updateValue(resultString!, forKey: index)
            parentAnswerArray.insert(resultString!, at: index)
        }
        var dictionary = Dictionary<String, String>()
        if isSubQuestion{
            var subDictionary = (subQuesAnsArray[index])! as [Int:String]
            let string = getNumericValueFromString(subDictionary[subIndex]!)
            dictionary = FincartCommon.getTaggedQuestionDictionary(jsonObject?.baseObject![index].quesArray?[subIndex].questioncode, goalCode: jsonObject?.baseObject![index].quesArray?[subIndex].goalcode, answer: string, child: 0)
        }else
        {
            let string = getNumericValueFromString(quesAnsArray[index]!)
            dictionary = FincartCommon.getTaggedQuestionDictionary(jsonObject?.baseObject![index].questioncode, goalCode: jsonObject?.baseObject![index].goalcode, answer: string, child: 0)
        }
        ...
        FincartCommon.refreshAccessToken(success: { (responseCode) in
            if responseCode == 200{
                if isReviewService{
                    ...
                    if self.isNextFlow{
                        self.tagQuestions(self.indexCount, subIndex: self.indexSubQuestionArray - 1, child: 0)
                    }else{
                        self.tagQuestions(self.indexCount, subIndex: self.indexSubQuestionArray, child: 0)
                    }
                }
            }else{
    ...
    {
        if questionFinished || (jsonObject?.baseObject?[indexCount].goalcode?.caseInsensitiveCompare("FG14") == ComparisonResult.orderedSame && quesAnsArray[indexCount]?.caseInsensitiveCompare("NO") == ComparisonResult.orderedSame){
            questionFinished = true
            FinCartMacros.showSVProgressHUD()
        }
        if isPreviousFlow{
            tagQuestions(indexCount, subIndex: indexSubQuestionArray, child: 0)
        }else{
            tagQuestions(indexCount, subIndex: indexSubQuestionArray - 1, child: 0)
        }
    }
